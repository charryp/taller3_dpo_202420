...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	while(iterador.hasNext() && !found)
    		aVuelo = iterador.next();
    		
    		if(fechaVuelo.equals(aVuelo.getFecha()) && codigoRuta.equals(aVuelo.getRuta().getCodigoRuta()))
    			found = true;
    ...
    {
        // TODO implementar
    	Collection<Tiquete> allTiquetes = new LinkedList<>();
    	Vuelo aVuelo = null;
    	Iterator<Vuelo> iterador = vuelos.iterator();
    	
    	while(iterador.hasNext())
    		aVuelo = iterador.next();
    		allTiquetes.addAll(aVuelo.getTiquetes());
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	Vuelo aVuelo = null;
    	Ruta rutaToProgram = null;
    	Ruta rutaAvuelo = null;
    	boolean avionIn = false;
    	Iterator<Vuelo> iterador = vuelos.iterator();
    	
    	while(iterador.hasNext())
    		aVuelo = iterador.next();
    		
    		if(nombreAvion.equals(aVuelo.getAvion().getNombre()))
    			avionIn = true;
    		
    			if(fecha.equals(aVuelo.getFecha()))
    			{
    				rutaToProgram = rutas.get(codigoRuta);
        			rutaAvuelo = aVuelo.getRuta();
        			int hRutaToProgram = Integer.parseInt(rutaToProgram.getHoraLlegada());
        			int hRutaAvuelo = Integer.parseInt(rutaAvuelo.getHoraLlegada());
        			
        			if(hRutaToProgram > hRutaAvuelo)
        				vuelos.add(new Vuelo(fecha, rutaToProgram, aVuelo.getAvion()));
    			}
    			else
    				vuelos.add(new Vuelo(fecha, rutaToProgram, aVuelo.getAvion()));
    	
    	if(!avionIn)
    		vuelos.add(new Vuelo(fecha, rutaToProgram, new Avion(nombreAvion, 0)));
    }
